...
Onlar sadece öbeğin durumunu değiştirmezler, özdeş yordamlar, sınıfın kendisine ```self .__class__``` özniteliği aracılığıyla da erişebilir. Bu, eşlenen yordamların sınıf durumunu da değiştirebileceği anlamına gelir. Not only can they modify object state, instance methods can also access the class itself through the self.__class__ attribute. This means instance methods can also modify class state.
