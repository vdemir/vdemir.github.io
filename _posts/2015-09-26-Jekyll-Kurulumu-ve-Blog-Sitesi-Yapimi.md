...
Kuruluma başlamadan önce hazırlık aşamasını tamamlamalıyız. Jekyll kurulumu için ```rubygems``` ve ```ruby-devel``` paketlerine ihtiyaç var. Önce paketlerin kurulumunu sağlıyoruz. Ayrıca ```gem install``` komutu ile ```jeklyy``, ```therubyracer``` ve ```rdiscount```
