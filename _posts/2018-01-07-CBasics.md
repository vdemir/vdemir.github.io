...
C'deki her değişken belirgin bir _type_ sahiptir. Bu tipler[type] kendi tarafımızdan bildirilir veya dile dahildirler. Yeni bir değişkeni ismini takiben onun tipini yazarak tanımlayabiliriz ve isteğe bağlı olarak onu `=` kullanarak bazı değerlere ayarlayabiliriz. Bu bildirim bir _statement_'dır ve C'deki tüm _statements_'leri noktalı virgül ';' ile sonlandırırız.
